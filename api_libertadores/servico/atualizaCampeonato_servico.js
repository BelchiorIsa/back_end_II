...
export async function atualizaCampeonato(id, campeao, vice, ano) {
    ...
    const [resposta] = await conexao.execute(query, [id, campeao, vice, ano, id]);
    console.log(resposta);
//id por ultimo pq tem que ser na ordem "UPDATE campeonatos SET campeao = ?, vice = ?, ano = ? WHERE id = ?";
    conexao.release();
    return resposta;
}
